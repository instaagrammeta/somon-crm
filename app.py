...
@app.route('/api/requests-bulk-paste', methods=['POST'])
@login_required
def bulk_paste_requests():
    """Илова кардани якчанд заявка ба колонкаи мавҷуда дар якҷоягӣ (paste)
    Заявкаҳои нав дар КИСМАТИ БОЛОИ колонка гузошта мешаванд (order_index пасттарин).
    """
    data = request.json
    ...
    if not column_id or not board_id or not items:
        return jsonify({'success': False, 'error': 'Маълумоти нокифоя'}), 400
    
    conn = get_db()
    try:
        # Гирифтани минимум order_index дар ин колонка
        min_order_row = conn.execute(
            'SELECT MIN(order_index) as min_order FROM requests_items WHERE column_id = ?',
            (column_id,)
        ).fetchone()
        min_order = min_order_row['min_order']
        if min_order is None:
            min_order = 0
        
        # Заявкаҳои нав аз min_order - len(items) сар мешаванд
        # то ҳамаашон болои ҳама бошанд
        start_order = min_order - len(items)
        
        new_ids = []
        now_iso = datetime.now().isoformat()
        
        for i, item in enumerate(items):
            # Иҷрокунанда - агар executor_id мавҷуд бошад, номашро гирем
            executor_id = item.get('executor_id')
            executor_name = item.get('executor_name', '') or ''
            if executor_id and not executor_name:
                user = conn.execute('SELECT full_name FROM users WHERE id = ?', (executor_id,)).fetchone()
                if user:
                    executor_name = user['full_name']
            
            files_value = item.get('files', [])
            # Агар files пешакӣ JSON-string бошад, мехонем
            if isinstance(files_value, str):
                try:
                    files_value = json.loads(files_value)
                except Exception:
                    files_value = []
            ...
            ''', (
                ...
                item.get('windows', 0),
                item.get('floor', 0),
                item.get('total_floors', 0),
                item.get('total_price', 0),
                item.get('price_per_m2', 0),
                item.get('phone', ''),
        ...
        conn.commit()
        return jsonify({'success': True, 'count': len(new_ids), 'ids': new_ids})
    except Exception as e:
        print(f"Error bulk_paste_requests: {e}")
        return jsonify({'success': False, 'error': str(e)}), 400
    finally:
        conn.close()


@app.route('/api/requests-column-export/<int:column_id>', methods=['GET'])
@login_required
def export_single_column_excel(column_id):
    """Экспорти заявкаҳои як колонка ба Excel"""
    conn = get_db()
    col = conn.execute('SELECT * FROM requests_columns WHERE id = ?', (column_id,)).fetchone()
    if not col:
        conn.close()
        return jsonify({'error': 'Колонка не найдена'}), 404
    
    query = '''
        SELECT r.*, u.full_name as author_name_x, u2.full_name as executor_name_x, c.title as column_title
        FROM requests_items r
        LEFT JOIN users u ON r.author_id = u.id
        LEFT JOIN users u2 ON r.executor_id = u2.id
        LEFT JOIN requests_columns c ON r.column_id = c.id
        WHERE r.column_id = ?
        ORDER BY r.order_index ASC, r.created_date DESC
    '''
    requests_rows = conn.execute(query, (column_id,)).fetchall()
    conn.close()
    ...
    for req in requests_rows:
        data.append({
            ...
            'Автор': req['author_name_x'] or '',
            'Иҷрокунанда': req['executor_name_x'] or '',
            'Шарҳ': req['comment'] or '',
    ...
    output = BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        sheet_name = (col['title'] or 'Колонка')[:30]
    ...
    output.seek(0)
    safe_title = "".join(c for c in (col['title'] or 'column') if c.isalnum() or c in (' ', '_', '-')).strip()
    return send_file(output,
                    download_name=f'column_{safe_title}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx',
                    as_attachment=True,
                    mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
